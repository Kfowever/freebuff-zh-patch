...
`dom-protection.cjs` 使用 DOM 桩，验证用户内容保护、输入框提示、嵌套执行标签及动态更新、工具标签和引用按钮，同时覆盖 0.0.109 的固定消息摘要、长消息展开控件、自定义服务商名称及 API 输入值保护。可另在浏览器中打开 `dom.html`，检查结果为 `PASS`，验证基础场景中的真实 DOM 和 MutationObserver 行为。这些测试不能代替 Freebuff 启动及界面兼容性检查。
